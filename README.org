...
GTK[fn:27] is a free and open-source, cross-platform widget toolkit for graphical user interfaces. It's one of the most popular toolkits for the Wayland[fn:20] and X11[fn:19] windowing systems.
...
Evil[fn:12] is an extensible VI layer for GNU/Emacs[fn:2]. It emulates the main features of Vim[fn:26], turning GNU/Emacs[fn:2] into a modal editor.
...
The next time Emacs[fn:2] is started, it will come up in /normal state/, denoted by =<N>= in the modeline. This is where the main ~vi~ bindings are defined. Like Emacs[fn:2] in general, Evil[fn:12] is extensible in Emacs Lisp[fn:25].
...
The Emacs Web Wowser[fn:31] is a Web browser written in Emacs Lisp[fn:25] based on the ~shr.el~ library. It's my primary browser when it comes to text-based browsing.
...
EShell [fn:36] is a shell-like command interpreter for GNU/Emacs[fn:2] implemented in Emacs Lisp[fn:25]. It invokes no external processes except for those requested by the user. It's intended to be an alternative for IELM, and a full REPL envionment for Emacs[fn:2].
...
Emacs Libvterm (VTerm)[fn:37] is a fully-fledged terminal emulator inside GNU/Emacs[fn:2] based on Libvterm[fn:38], a blazing fast C library used in Neovim[fn:26]. As a result of using compiled code (instead of Emacs Lisp[fn:25]), VTerm[fn:37] is capable, fast, and it can seamlessly handle large outputs.
...
Go Mode[fn:60] is a major mode for editing Golang[fn:14] source code in GNU/Emacs[fn:2].
...
The built in Python Mode[fn:61] has a nice feature set for working with Python[fn:18] code in GNU/Emacs[fn:2]. It is complimented with the addition of an LSP[fn:58] server. These tools are included in the Development Shell[fn:13] for Python[fn:18].
