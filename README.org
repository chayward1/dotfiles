...
NixOS[fn:2] typically stores the current machine configuration in =/etc/nixos/configuration.nix=. In this project, this file is stored in =/etc/dotfiles/hosts/$HOSTNAME/...=, and imported, along with the generated hardware configurations. This ensures that multiple host machines can share the same modules, and generating new host definitions is trivial. It also makes it easier to share common configurations amongst all of the hosts, such as with pre-configured wireless networking:
...
The default host, built using QEMU[fn:3], a free and open-source emulator that can perform hardware virtualization. It features a lightweight system optimized for development, running GNU/Emacs[fn:4] + EXWM[fn:5] as the graphical environment.
...
This is a basic default configuration that specified the indended default configuration of the system. Because NixOS[fn:2] has a declarative configuration model, you can create or edit a description of the desired configuration, and update it from one file.
...
The file system for this host is a single 24GB QCOW file, a format for disk images used by QEMU[fn:3]. The file can be recreated easily by following the steps listed in the NixOS[fn:2] installation manual, specifically the section on disk formatting.
...
My gaming laptop, the model is an Acer Nitro AN-515-53[fn:6]. The Nitro 5 has more in common with the mid-range notebooks rather than the gaming models due to its cooling design, chassis, and overall construction.
...
The Raspberry Pi 400[fn:7] is your complete personal computer, built into a compact keyboard. It features a quad-core, 64-bit processor, 4GB of RAM, wireless networking, dual-display output, 4k video playback, as well as a 40-pin GPIO header. It's the most powerful Raspberry Pi computer yet.
...
The Raspberry Pi Model B-8GB[fn:8] is the latest product in the popular Raspberry Pi range of computers. It offers groundbreaking increases in processor speed, multimedia performance, memory, and connectivity compared to the prior generation. On NixOS[fn:2], the Raspberry Pi family is /only/ supported on the =AArch64= platform, although there is community support for =armv6l= and =armv7l=.
...
This is my Samsung Galaxy S10+[fn:9] running Nix On Droid[fn:10] with the experimental support for Flakes being used to manage the configuration.
...
Modules are files combined by NixOS[fn:2] to produce the full system configuration. Modules wre introduced to allow extending NixOS[fn:2] without modifying its source code. They also allow splitting up =configuration.nix=, making the system configuration easier to maintain and use.
...
X11, or X[fn:11] is the generic name for the X Window System Display Server. All graphical GNU/Linux[fn:12] applications connect to an X-Window[fn:11] (or Wayland[fn:13]) to display graphical data on the monitor of a computer. Its a program that acts as the interface between graphical applications and the graphics subsystem of the computer.
...
OpenSSH[fn:14] is a suite of secure networking utilities based on the Secure Shell Protocol, which provides a secure channel over an unsecured network in a client-server architecture. OpenSSH[fn:14] started as a fork of the free SSH program; later versions were proprietary software.
...
Hugo[fn:15] is one of the most popular open-source static site generators.
...
Nix Flakes[fn:16] are an upcoming feature of the Nix package manager[fn:17]. They allow you to specify your codes dependencies in a declarative way, simply by listing them inside of a ~flake.nix~ file. Each dependency is then pinned to a specific git-hash. Flakes[fn:16] replace the =nix-channels= command and things like ~builtins.fetchGit~, keeping dependencies at the top of the tree, and channels always in sync. Currently, Flakes[fn:16] are not available unless explicitly enabled.
...
Cachix[fn:18] is a Command line client for Nix[fn:17] binary cache hosting. This allows downloading and usage of pre-compiled binaries for applications on /nearly/ every available system architecture. This speeds up the time it takes to rebuild configurations.
...
Use the ~lspci~ command to determine the type of graphics card you have, following the guide on NVIDIA at the NixOS Wiki[fn:19].
...
Firefox Browser[fn:20], also known as Mozilla Firefox or simply Firefox, is a free and open-source web browser developed by the Mozilla Foundation and its subsidiary, the Mozilla Corporation. Firefox uses the Gecko layout engine to render web pages, which implements current and anticipated web standards. In 2017, Firefox began incorporating new technology under the code name Quantum to promote parallelism and a more intuitive user interface.
...
Moonlight[fn:22] is an open-source implementation of NVIDIA's GameStream Protocol. You can stream your collection of PC games from your GameStream-compatible PC to any supported device and play them remotely. Moonlight[fn:22] is perfect for gaming on the go (or on GNU/Linux[fn:12]) without sacrificing the graphics and game selection available for the PC.
...
The Teamviewer[fn:23] remote connectivity cloud platform enables secure remote access to any device, across platforms, from anywhere, anytime. Teamviewer[fn:23] connects computers, smartphones, servers, IoT devices, robots -- anything -- with fast, high performance connections through their global access network. It has been used in outer-space low-bandwidth environments.
...
Home Manager[fn:24] includes a =flake.nix= file for compatibility with Nix Flakes, a feature utilized heavily in this project. When using flakes, switching to a new configuration is done /only/ for the entire system, using the command ~nixos-rebuild switch --flake <path>~, instead of ~nixos-rebuild~, and ~home-manager~ seperately.
...
GNU Privacy Guard[fn:26] is a free-software replacement for Symantec's PGP cryptographic software suite. It is compliant with RFC 4880, the IETF standards-track specification of OpenPGP. Modern versions of PGP are interoperable with GnuPG and other OpenPGP-compliant systems.
...
Neovim[fn:27] is a project that seeks to aggressively refactor Vim in order to:
...
GTK[fn:28] is a free and open-source, cross-platform widget toolkit for graphical user interfaces. It's one of the most popular toolkits for the Wayland[fn:13] and X11[fn:11] windowing systems.
...
GNU/Emacs[fn:4] is an extensible, customizable, free/libre text editor -- and more. At its core is an interpreter for Emacs Lisp[fn:26], a dialect of the Lisp programming language with extensions to support text editing. Other features include:
...
Emacs[fn:4] has been around since the 1980s, and it's painfully obvious when you're greeted with the default user interface. Disable some unwanted features to clean it up, and bring the appearance to something closer to a modern editor.
...
Emacs[fn:4] makes a backup for a file only the first time the file is saved from a buffer. No matter how many times the file is subsequently written to, the backup remains unchanged. For files managed by a version control system, backup files are redundant since the previous versions are already stored.
...
Define some methods for interaction between GNU/Emacs[fn:4], and the systems underyling shell:

1) Method to run an external process, launching any application on a new process without interferring with Emacs[fn:4]
2) Method to apply commands to the curren call process, effecting the running instance of Emacs[fn:4]
...
Nix-mode[fn:29] is an Emacs[fn:4] major mode for editing Nix[fn:17] expressions. This provides basic handling of =.nix= files. Syntax highlighting and indentation support using =SMIE= are provided.
...
Evil[fn:30] is an extensible VI layer for GNU/Emacs[fn:4]. It emulates the main features of Vim[fn:27], turning GNU/Emacs[fn:4] into a modal editor.
...
The next time Emacs[fn:4] is started, it will come up in /normal state/, denoted by =<N>= in the modeline. This is where the main ~vi~ bindings are defined. Like Emacs[fn:4] in general, Evil[fn:30] is extensible in Emacs Lisp[fn:26].
...
EXWM (Emacs X Window Manager)[fn:5] is a full-featured tiling X window manager for GNU/Emacs[fn:4] built on-top of XELB. It features:
...
I wanted to leave ~(exwm-enable)~ out of my Emacs configuration (which does no harm anyways). This can be called when using the daemon to start EXWM[fn:5].
...
EXWM[fn:5] cannot make an X window manager by itself, this is by design; You must tell X to do it. Override the =~/.xinitrc= file to start the =xsession=.
...
General[fn:31] provides a more convenient method for binding keys in Emacs[fn:4], providing a unified interface for key definitions. Its primary purpose is to build on /existing/ functionality to make key definitions more clear and concise.
...
Which-key[fn:32] is a minor mode for Emacs[fn:4] that displays the key bindings following your currently entered incomplete command (prefix) in a popup or mini-buffer.
...
The Emacs Web Wowser[fn:33] is a Web browser written in Emacs Lisp[fn:26] based on the ~shr.el~ library. It's my primary browser when it comes to text-based browsing.
...
ERC[fn:34] is a powerful, modular, and extensible IRC client for GNU/Emacs[fn:4]. It's part of the GNU project, and included in Emacs.
...
Dired[fn:35] shows a directory listing inside of an Emacs[fn:4] buffer that can be used to perform various file operations on files and subdirectories. THe operations you can perform are numerous, from creating subdirectories, byte-compiling files, searching, and editing files. Dired-Extra[fn:36] provides extra functionality for Dired[fn:35].
...
All The Icons[fn:37] is a utility package to collect various Icon Fonts and prioritize them within GNU/Emacs[fn:4].
...
Emojify[fn:38] is an Emacs[fn:4] extension to display Emojis. It can display GitHub style Emojis like :smile: or plain ascii ones such as :). It tries to be as efficient as possible, while also providing flexibility.
...
EShell [fn:39] is a shell-like command interpreter for GNU/Emacs[fn:4] implemented in Emacs Lisp[fn:26]. It invokes no external processes except for those requested by the user. It's intended to be an alternative for IELM, and a full REPL envionment for Emacs[fn:4].
...
Emacs Libvterm (VTerm)[fn:40] is a fully-fledged terminal emulator inside GNU/Emacs[fn:4] based on Libvterm[fn:41], a blazing fast C library used in Neovim[fn:27]. As a result of using compiled code (instead of Emacs Lisp[fn:26]), VTerm[fn:40] is capable, fast, and it can seamlessly handle large outputs.
...
Magit[fn:42] is an interface to the Git[fn:25] version control system, implemented as a GNU/Emacs[fn:4] package written in Elisp[fn:26]. It fills the glaring gap between the Git[fn:25] command line interface and various GUIs, letting you perform trivial as well as elaborate version control tasks within a few mnemonic key presses.
...
Elfeed[fn:43] is an extensible web feed reader for GNU/Emacs[fn:4], support both =Atom= and =RSS=. It requires =Emacs 24.3+= and is available for download from the standard repositories.
...
Org-mode[fn:44] is a document editing and organizing mode, designed for notes, planning, and authoring within the free software text editor GNU/Emacs[fn:4]. The name is used to encompass plain text files (such as this one) that include simple marks to indicate levels of a hierarchy, and an editor with functions that can read the markup and manipulate the hierarchy elements.
...
Org Roam[fn:45] is a plain-text knowledge management system. It borrows principles from the Zettelkasten method[fn:46], providing a solution for non-hierarchical note-taking. It should also work as a plug-and-play solution for anyone already using Org Mode[fn:44] for their personal wiki (me).
...
Org Drill[fn:47] is an extension for Org Mode[fn:44] that uses a spaced repition algorithm to conduct interactive /Drill Sessions/ using Org files as sources of facts to be memorized.
...
The way Org Mode[fn:44] works, TODO items, time-stamped items, and tagged headlines can be scattered throughout a file, or even a number of files. To get an overview of open action items, or of events that are important for a particular date, this information must be collected, sorted, and displayed in an organized way.
...
Writegood Mode[fn:50] is an Emacs[fn:4] minor mode to aid in finding common writing problems. It highlights the text based on the following criteria:
...
It's possible to make HTTP requests from Org Mode buffers using ob-http[fn:54], this relies on Org Babel being present and configured properly.
...
Ox-Hugo[fn:55] is an Org-Mode[fn:44] exporter for Hugo[fn:15] compabile markdown. My dotfiles are a result of this, and are available to view here https://chrishayward.xyz/dotfiles/.
...
Manage Docker from inside of Emacs using Docker.el[fn:57]. This is a full docker porcelain similar to Magit, allowing complete control of a Docker system. Add syntax highlighting to Dockerfiles using dockerfile-mode[fn:58] from Spotify.
...
Projectile[fn:59] is a project interaction library for GNU/Emacs[fn:4]. Its goal is to provide a nice set of features operating on a project level, without introducing external dependencies.
...
Emacs CCLS[fn:61] is a client for CCLS, a C/C++/Objective-C language server supporting multi-million line C++ code bases, powered by libclang.
...
Company[fn:62] is a text completion framework for GNU/Emacs[fn:4]. The name stands for =Complete Anything=. It uses pluggable back-ends and front-ends to retieve and display completion candidates.
...
Go Mode[fn:63] is a major mode for editing Golang[fn:64] source code in GNU/Emacs[fn:4].
...
Rustic[fn:62] is a fork of rust-mode that integrates well with the Language Server Protocol[fn:56]. Include the rust shell before launching GNU/Emacs[fn:4] to use this!
...
The built in Python Mode[fn:65] has a nice feature set for working with Python[fn:66] code in GNU/Emacs[fn:4]. It is complimented with the addition of an LSP[fn:60] server. These tools are included in the Development Shell[fn:67] for Python[fn:66].
...
PlantUML Mode[fn:69] is a major mode for editing PlantUML[fn:68] sources in GNU/Emacs[fn:4].
...
Ivy (Swiper)[fn:70] is a generic completion mechanism for GNU/Emacs[fn:4]. While operating similarily to other completion schemes like =icomplete-mode=, it aims to be more efficient, smaller, simpler, and smoother to use, while remaining highly customizable.
...
It's possible to control the frame opacity in GNU/Emacs[fn:4]. Unlike other transparency hacks, it's not merely showing the desktop background image, but is true transparency -- you can se other windows behind the Emacs[fn:4] window.
...
The Desktop Environment[fn:71] package provides commands and a global minor mode for controlling your GNU/Linux[fn:12] desktop from within GNU/Emacs[fn:4].
...
Doom Themes[fn:72] is a theme megapack for GNU/Emacs[fn:4], inspired by community favourites.
...
[fn:1] https://heroku.com

[fn:2] https://nixos.org/manual/nixos/stable

[fn:3] https://qemu.org

[fn:4] https://gnu.org/software/emacs/

[fn:5] https://github.com/ch11ng/exwm

[fn:6] https://laptopmedia.com/laptop-specs/acer-nitro-5-an515-53-2

[fn:7] https://raspberrypi.org/products/raspberry-pi-400/

[fn:8] https://www.raspberrypi.org/products/raspberry-pi-4-model-b/

[fn:9] https://samsung.com/us/mobile/galaxy-s10/buy/
...
[fn:13] https://wayland.freedesktop.org

[fn:14] https://openssh.com

[fn:15] https://gohugo.io

[fn:16] https://nixos.wiki/wiki/Flakes

[fn:17] https://nixos.org/manual/nix/stable

[fn:18] https://nix-community.cachix.org

[fn:19] https://nixos.wiki/wiki/Nvidia

[fn:20] https://en.wikipedia.org/wiki/Firefox

[fn:21] https://jellyfin.org

[fn:22] https://moonlight-stream.org

[fn:23] https://teamviewer.com

[fn:24] https://github.com/nix-community/home-manager

[fn:25] https://git-scm.com

[fn:26] https://emacswiki.org/emacs/LearnEmacsLisp

[fn:27] https://neovim.io

[fn:28] https://gtk.org

[fn:29] https://github.com/nixos/nix-mode

[fn:30] https://evil.readthedocs.io/en/latest/overview.html
...
[fn:34] https://gnu.org/software/emacs/erc.html

[fn:35] https://emacswiki.org/emacs/DiredMode

[fn:36] https://emacswiki.org/emacs/DiredExtra#Dired_X

[fn:37] https://github.com/domtronn/all-the-icons.el

[fn:38] https://github.com/iqbalansari/emacs-emojify

[fn:39] https://gnu.org/software/emacs/manual/html_mono/eshell.html

[fn:40] https://github.com/akermu/emacs-libvterm

[fn:41] https://github.com/neovim/libvterm

[fn:42] https://magit.vc

[fn:43] https://github.com/skeeto/elfeed

[fn:44] https://orgmode.org

[fn:45] https://github.com/org-roam/org-roam

[fn:46] https://zettelkasten.de

[fn:47] https://orgmode.org/worg/org-contrib/org-drill.html

[fn:48] https://github.com/marcinkoziej/org-pomodoro
...
[fn:52] https://wikipedia.org/wiki/Tex_Live

[fn:53] https://nixos.wiki/wiki/TexLive

[fn:54] https://github.com/zweifisch/ob-http

[fn:55] https://oxhugo.scripter.co

[fn:56] https://password-store.org

[fn:57] https://github.com/Silex/docker.el

[fn:58] https://github.com/spotify/dockerfile-mode

[fn:59] https://projectile.mx

[fn:60] https://microsoft.github.io/language-server-protocol

[fn:61] https://github.com/MaskRay/emacs-ccls

[fn:62] https://company-mode.github.io

[fn:63] https://emacswiki.org/emacs/GoMode

[fn:64] https://golang.org

[fn:65] https://plantuml.com

[fn:66] https://python.org

[fn:67] https://nixos.org/manual/nix/unstable/command-ref/nix-shell.html

[fn:68] https://github.com/skuro/plantuml-mode

[fn:69] https://github.com/abo-abo/swiper

[fn:70] https://github.com/DamienCassou/desktop-environment

[fn:71] https://github.com/hlissner/emacs-doom-themes

[fn:72] https://github.com/seagle0128/doom-modeline

[fn:73] https://grpc.io

[fn:74] https://iso.org/standard/74528.html

[fn:75] https://en.wikipedia.org/wiki/C++/

[fn:76] https://github.com/nix-community/emacs-overlay

[fn:77] https://github.com/nixos/nixos-hardware

[fn:78] https://nixos.org/manual/nixpkgs/stable

[fn:79] https://literateprogramming.com/knuthweb.pdf

[fn:80] https://openjdk.java.net

[fn:81] https://nodejs.org

[fn:82] https://rust-lang.org

[fn:83] https://docs.docker.com/machine/

[fn:84] https://docker.org

[fn:85] https://docs.docker.com/compose/
