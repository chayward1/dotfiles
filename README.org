...
Firefox Browser[fn:32], also known as Mozilla Firefox or simply Firefox, is a free and open-source web browser developed by the Mozilla Foundation and its subsidiary, the Mozilla Corporation. Firefox uses the Gecko layout engine to render web pages, which implements current and anticipated web standards. In 2017, Firefox began incorporating new technology under the code name Quantum to promote parallelism and a more intuitive user interface.
...
Moonlight[fn:34] is an open-source implementation of NVIDIA's GameStream Protocol. You can stream your collection of PC games from your GameStream-compatible PC to any supported device and play them remotely. Moonlight[fn:34] is perfect for gaming on the go (or on GNU/Linux[fn:1]) without sacrificing the graphics and game selection available for the PC.
...
The Teamviewer[fn:35] remote connectivity cloud platform enables secure remote access to any device, across platforms, from anywhere, anytime. Teamviewer[fn:35] connects computers, smartphones, servers, IoT devices, robots -- anything -- with fast, high performance connections through their global access network. It has been used in outer-space low-bandwidth environments.
...
GTK[fn:39] is a free and open-source, cross-platform widget toolkit for graphical user interfaces. It's one of the most popular toolkits for the Wayland[fn:26] and X11[fn:25] windowing systems.
...
GNU/Emacs[fn:2] is an extensible, customizable, free/libre text editor -- and more. At its core is an interpreter for Emacs Lisp[fn:37], a dialect of the Lisp programming language with extensions to support text editing. Other features include:
...
Nix-mode[fn:40] is an Emacs[fn:2] major mode for editing Nix[fn:5] expressions. This provides basic handling of =.nix= files. Syntax highlighting and indentation support using =SMIE= are provided.
...
Evil[fn:21] is an extensible VI layer for GNU/Emacs[fn:2]. It emulates the main features of Vim[fn:38], turning GNU/Emacs[fn:2] into a modal editor.
...
The next time Emacs[fn:2] is started, it will come up in /normal state/, denoted by =<N>= in the modeline. This is where the main ~vi~ bindings are defined. Like Emacs[fn:2] in general, Evil[fn:21] is extensible in Emacs Lisp[fn:37].
...
General[fn:41] provides a more convenient method for binding keys in Emacs[fn:2], providing a unified interface for key definitions. Its primary purpose is to build on /existing/ functionality to make key definitions more clear and concise.
...
Which-key[fn:42] is a minor mode for Emacs[fn:2] that displays the key bindings following your currently entered incomplete command (prefix) in a popup or mini-buffer.
...
The Emacs Web Wowser[fn:43] is a Web browser written in Emacs Lisp[fn:37] based on the ~shr.el~ library. It's my primary browser when it comes to text-based browsing.
...
ERC[fn:44] is a powerful, modular, and extensible IRC client for GNU/Emacs[fn:2]. It's part of the GNU project, and included in Emacs.
...
Dired[fn:45] shows a directory listing inside of an Emacs[fn:2] buffer that can be used to perform various file operations on files and subdirectories. THe operations you can perform are numerous, from creating subdirectories, byte-compiling files, searching, and editing files. Dired-Extra[fn:46] provides extra functionality for Dired[fn:45].
...
All The Icons[fn:47] is a utility package to collect various Icon Fonts and prioritize them within GNU/Emacs[fn:2].
...
Emojify[fn:48] is an Emacs[fn:2] extension to display Emojis. It can display GitHub style Emojis like :smile: or plain ascii ones such as :). It tries to be as efficient as possible, while also providing flexibility.
...
EShell [fn:49] is a shell-like command interpreter for GNU/Emacs[fn:2] implemented in Emacs Lisp[fn:37]. It invokes no external processes except for those requested by the user. It's intended to be an alternative for IELM, and a full REPL envionment for Emacs[fn:2].
...
Emacs Libvterm (VTerm)[fn:50] is a fully-fledged terminal emulator inside GNU/Emacs[fn:2] based on Libvterm[fn:51], a blazing fast C library used in Neovim[fn:38]. As a result of using compiled code (instead of Emacs Lisp[fn:37]), VTerm[fn:50] is capable, fast, and it can seamlessly handle large outputs.
...
Magit[fn:52] is an interface to the Git[fn:36] version control system, implemented as a GNU/Emacs[fn:2] package written in Elisp[fn:37]. It fills the glaring gap between the Git[fn:36] command line interface and various GUIs, letting you perform trivial as well as elaborate version control tasks within a few mnemonic key presses.
...
Elfeed[fn:53] is an extensible web feed reader for GNU/Emacs[fn:2], support both =Atom= and =RSS=. It requires =Emacs 24.3+= and is available for download from the standard repositories.
...
Org-mode[fn:54] is a document editing and organizing mode, designed for notes, planning, and authoring within the free software text editor GNU/Emacs[fn:2]. The name is used to encompass plain text files (such as this one) that include simple marks to indicate levels of a hierarchy, and an editor with functions that can read the markup and manipulate the hierarchy elements.
...
Org Roam[fn:55] is a plain-text knowledge management system. It borrows principles from the Zettelkasten method[fn:56], providing a solution for non-hierarchical note-taking. It should also work as a plug-and-play solution for anyone already using Org Mode[fn:54] for their personal wiki (me). Org Roam Server[fn:57] is a Web application to visualize the Org Roam[fn:55] database. Although it should automatically reload if there's a change in the database, it can be done so manually by clicking the =reload= button on the Web interface.
...
Org Drill[fn:58] is an extension for Org Mode[fn:54] that uses a spaced repition algorithm to conduct interactive /Drill Sessions/ using Org files as sources of facts to be memorized.
...
The way Org Mode[fn:54] works, TODO items, time-stamped items, and tagged headlines can be scattered throughout a file, or even a number of files. To get an overview of open action items, or of events that are important for a particular date, this information must be collected, sorted, and displayed in an organized way.
...
Writegood Mode[fn:61] is an Emacs[fn:2] minor mode to aid in finding common writing problems. It highlights the text based on the following criteria:
...
Ox-Hugo[fn:63] is an Org-Mode[fn:54] exporter for Hugo[fn:28] compabile markdown. I post nonsense on my Personal Blog[fn:64], and share my notes on various textbooks, articles, and software Here[fn:65].
...
Reveal.js[fn:66] is an open source HTML presentation framework. It enables anyone with a web browser to create fully-featured and beautiful presentations for free. Presentations with Reveal.js[fn:66] are built on open web technologies. That means anything you can do on the web, you can do in your presentation. Ox Reveal[fn:67] is an Org Mode[fn:54] export backend.
...
[fn:32] https://en.wikipedia.org/wiki/Firefox

[fn:33] https://jellyfin.org

[fn:34] https://moonlight-stream.org

[fn:35] https://teamviewer.com
...
[fn:41] https://github.com/noctuid/general.el

[fn:42] https://github.com/justbur/emacs-which-key

[fn:43] https://emacswiki.org/emacs/eww

[fn:44] https://gnu.org/software/emacs/erc.html

[fn:45] https://emacswiki.org/emacs/DiredMode

[fn:46] https://emacswiki.org/emacs/DiredExtra#Dired_X

[fn:47] https://github.com/domtronn/all-the-icons.el

[fn:48] https://github.com/iqbalansari/emacs-emojify

[fn:49] https://gnu.org/software/emacs/manual/html_mono/eshell.html

[fn:50] https://github.com/akermu/emacs-libvterm

[fn:51] https://github.com/neovim/libvterm

[fn:52] https://magit.vc

[fn:53] https://github.com/skeeto/elfeed

[fn:54] https://orgmode.org

[fn:55] https://github.com/org-roam/org-roam

[fn:56] https://zettelkasten.de

[fn:57] https://github.com/org-roam/org-roam-server
