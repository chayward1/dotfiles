...
My gaming laptop, the model is an Acer Nitro AN-515-53[fn:1]. The Nitro 5 has more in common with the mid-range notebooks rather than the gaming models due to its cooling design, chassis, and overall construction.
...
The Raspberry Pi 400[fn:2] is your complete personal computer, built into a compact keyboard. It features a quad-core, 64-bit processor, 4GB of RAM, wireless networking, dual-display output, 4k video playback, as well as a 40-pin GPIO header. It's the most powerful Raspberry Pi computer yet.
...
This is my Samsung Galaxy S10+[fn:5] running Nix On Droid[fn:6] with the experimental support for Flakes being used to manage the configuration.
...
X11, or X[fn:7] is the generic name for the X Window System Display Server. All graphical GNU/Linux[fn:8] applications connect to an X-Window[fn:7] (or Wayland[fn:9]) to display graphical data on the monitor of a computer. Its a program that acts as the interface between graphical applications and the graphics subsystem of the computer.
...
OpenSSH[fn:10] is a suite of secure networking utilities based on the Secure Shell Protocol, which provides a secure channel over an unsecured network in a client-server architecture. OpenSSH[fn:10] started as a fork of the free SSH program; later versions were proprietary software.
...
Hugo[fn:11] is one of the most popular open-source static site generators.
...
Nix Flakes[fn:12] are an upcoming feature of the Nix package manager[fn:13]. They allow you to specify your codes dependencies in a declarative way, simply by listing them inside of a ~flake.nix~ file. Each dependency is then pinned to a specific git-hash. Flakes[fn:12] replace the =nix-channels= command and things like ~builtins.fetchGit~, keeping dependencies at the top of the tree, and channels always in sync. Currently, Flakes[fn:12] are not available unless explicitly enabled.
...
Cachix[fn:14] is a Command line client for Nix[fn:13] binary cache hosting. This allows downloading and usage of pre-compiled binaries for applications on /nearly/ every available system architecture. This speeds up the time it takes to rebuild configurations.
...
Use the ~lspci~ command to determine the type of graphics card you have, following the guide on NVIDIA at the NixOS Wiki[fn:15].
...
Firefox Browser[fn:16], also known as Mozilla Firefox or simply Firefox, is a free and open-source web browser developed by the Mozilla Foundation and its subsidiary, the Mozilla Corporation. Firefox uses the Gecko layout engine to render web pages, which implements current and anticipated web standards. In 2017, Firefox began incorporating new technology under the code name Quantum to promote parallelism and a more intuitive user interface.
...
Moonlight[fn:18] is an open-source implementation of NVIDIA's GameStream Protocol. You can stream your collection of PC games from your GameStream-compatible PC to any supported device and play them remotely. Moonlight[fn:18] is perfect for gaming on the go (or on GNU/Linux[fn:8]) without sacrificing the graphics and game selection available for the PC.
...
The Teamviewer[fn:19] remote connectivity cloud platform enables secure remote access to any device, across platforms, from anywhere, anytime. Teamviewer[fn:19] connects computers, smartphones, servers, IoT devices, robots -- anything -- with fast, high performance connections through their global access network. It has been used in outer-space low-bandwidth environments.
...
Home Manager[fn:20] includes a =flake.nix= file for compatibility with Nix Flakes, a feature utilized heavily in this project. When using flakes, switching to a new configuration is done /only/ for the entire system, using the command ~nixos-rebuild switch --flake <path>~, instead of ~nixos-rebuild~, and ~home-manager~ seperately.
...
GNU Privacy Guard[fn:22] is a free-software replacement for Symantec's PGP cryptographic software suite. It is compliant with RFC 4880, the IETF standards-track specification of OpenPGP. Modern versions of PGP are interoperable with GnuPG and other OpenPGP-compliant systems.
...
Neovim[fn:23] is a project that seeks to aggressively refactor Vim in order to:
...
GTK[fn:24] is a free and open-source, cross-platform widget toolkit for graphical user interfaces. It's one of the most popular toolkits for the Wayland[fn:9] and X11[fn:7] windowing systems.
...
GNU/Emacs[fn:25] is an extensible, customizable, free/libre text editor -- and more. At its core is an interpreter for Emacs Lisp[fn:22], a dialect of the Lisp programming language with extensions to support text editing. Other features include:
...
Emacs[fn:25] has been around since the 1980s, and it's painfully obvious when you're greeted with the default user interface. Disable some unwanted features to clean it up, and bring the appearance to something closer to a modern editor.
...
Emacs[fn:25] makes a backup for a file only the first time the file is saved from a buffer. No matter how many times the file is subsequently written to, the backup remains unchanged. For files managed by a version control system, backup files are redundant since the previous versions are already stored.
...
Define some methods for interaction between GNU/Emacs[fn:25], and the systems underyling shell:

1) Method to run an external process, launching any application on a new process without interferring with Emacs[fn:25]
2) Method to apply commands to the curren call process, effecting the running instance of Emacs[fn:25]
...
Nix-mode[fn:26] is an Emacs[fn:25] major mode for editing Nix[fn:13] expressions. This provides basic handling of =.nix= files. Syntax highlighting and indentation support using =SMIE= are provided.
...
Evil[fn:27] is an extensible VI layer for GNU/Emacs[fn:25]. It emulates the main features of Vim[fn:23], turning GNU/Emacs[fn:25] into a modal editor.
...
The next time Emacs[fn:25] is started, it will come up in /normal state/, denoted by =<N>= in the modeline. This is where the main ~vi~ bindings are defined. Like Emacs[fn:25] in general, Evil[fn:27] is extensible in Emacs Lisp[fn:22].
...
EXWM (Emacs X Window Manager)[fn:28] is a full-featured tiling X window manager for GNU/Emacs[fn:25] built on-top of XELB. It features:
...
I wanted to leave ~(exwm-enable)~ out of my Emacs configuration (which does no harm anyways). This can be called when using the daemon to start EXWM[fn:28].
...
EXWM[fn:28] cannot make an X window manager by itself, this is by design; You must tell X to do it. Override the =~/.xinitrc= file to start the =xsession=.
...
General[fn:29] provides a more convenient method for binding keys in Emacs[fn:25], providing a unified interface for key definitions. Its primary purpose is to build on /existing/ functionality to make key definitions more clear and concise.
...
Which-key[fn:30] is a minor mode for Emacs[fn:25] that displays the key bindings following your currently entered incomplete command (prefix) in a popup or mini-buffer.
...
The Emacs Web Wowser[fn:31] is a Web browser written in Emacs Lisp[fn:22] based on the ~shr.el~ library. It's my primary browser when it comes to text-based browsing.
...
ERC[fn:32] is a powerful, modular, and extensible IRC client for GNU/Emacs[fn:25]. It's part of the GNU project, and included in Emacs.
...
Dired[fn:33] shows a directory listing inside of an Emacs[fn:25] buffer that can be used to perform various file operations on files and subdirectories. THe operations you can perform are numerous, from creating subdirectories, byte-compiling files, searching, and editing files. Dired-Extra[fn:34] provides extra functionality for Dired[fn:33].
...
All The Icons[fn:35] is a utility package to collect various Icon Fonts and prioritize them within GNU/Emacs[fn:25].
...
Emojify[fn:36] is an Emacs[fn:25] extension to display Emojis. It can display GitHub style Emojis like :smile: or plain ascii ones such as :). It tries to be as efficient as possible, while also providing flexibility.
...
EShell [fn:37] is a shell-like command interpreter for GNU/Emacs[fn:25] implemented in Emacs Lisp[fn:22]. It invokes no external processes except for those requested by the user. It's intended to be an alternative for IELM, and a full REPL envionment for Emacs[fn:25].
...
Emacs Libvterm (VTerm)[fn:38] is a fully-fledged terminal emulator inside GNU/Emacs[fn:25] based on Libvterm[fn:39], a blazing fast C library used in Neovim[fn:23]. As a result of using compiled code (instead of Emacs Lisp[fn:22]), VTerm[fn:38] is capable, fast, and it can seamlessly handle large outputs.
...
Magit[fn:40] is an interface to the Git[fn:21] version control system, implemented as a GNU/Emacs[fn:25] package written in Elisp[fn:22]. It fills the glaring gap between the Git[fn:21] command line interface and various GUIs, letting you perform trivial as well as elaborate version control tasks within a few mnemonic key presses.
...
Elfeed[fn:41] is an extensible web feed reader for GNU/Emacs[fn:25], support both =Atom= and =RSS=. It requires =Emacs 24.3+= and is available for download from the standard repositories.
...
Org-mode[fn:42] is a document editing and organizing mode, designed for notes, planning, and authoring within the free software text editor GNU/Emacs[fn:25]. The name is used to encompass plain text files (such as this one) that include simple marks to indicate levels of a hierarchy, and an editor with functions that can read the markup and manipulate the hierarchy elements.
...
Org Roam[fn:43] is a plain-text knowledge management system. It borrows principles from the Zettelkasten method[fn:44], providing a solution for non-hierarchical note-taking. It should also work as a plug-and-play solution for anyone already using Org Mode[fn:42] for their personal wiki (me).
...
Org Drill[fn:45] is an extension for Org Mode[fn:42] that uses a spaced repition algorithm to conduct interactive /Drill Sessions/ using Org files as sources of facts to be memorized.
...
The way Org Mode[fn:42] works, TODO items, time-stamped items, and tagged headlines can be scattered throughout a file, or even a number of files. To get an overview of open action items, or of events that are important for a particular date, this information must be collected, sorted, and displayed in an organized way.
...
Writegood Mode[fn:48] is an Emacs[fn:25] minor mode to aid in finding common writing problems. It highlights the text based on the following criteria:
...
It's possible to make HTTP requests from Org Mode buffers using ob-http[fn:52], this relies on Org Babel being present and configured properly.
...
Ox-Hugo[fn:53] is an Org-Mode[fn:42] exporter for Hugo[fn:11] compabile markdown. My dotfiles are a result of this, and are available to view here https://chrishayward.xyz/dotfiles/.
...
Manage Docker from inside of Emacs using Docker.el[fn:55]. This is a full docker porcelain similar to Magit, allowing complete control of a Docker system. Add syntax highlighting to Dockerfiles using dockerfile-mode[fn:56] from Spotify.
...
Projectile[fn:57] is a project interaction library for GNU/Emacs[fn:25]. Its goal is to provide a nice set of features operating on a project level, without introducing external dependencies.
...
Emacs CCLS[fn:59] is a client for CCLS, a C/C++/Objective-C language server supporting multi-million line C++ code bases, powered by libclang.
...
Company[fn:60] is a text completion framework for GNU/Emacs[fn:25]. The name stands for =Complete Anything=. It uses pluggable back-ends and front-ends to retieve and display completion candidates.
...
Go Mode[fn:61] is a major mode for editing Golang[fn:62] source code in GNU/Emacs[fn:25].
...
Rustic[fn:60] is a fork of rust-mode that integrates well with the Language Server Protocol[fn:54]. Include the rust shell before launching GNU/Emacs[fn:25] to use this!
...
The built in Python Mode[fn:63] has a nice feature set for working with Python[fn:64] code in GNU/Emacs[fn:25]. It is complimented with the addition of an LSP[fn:58] server. These tools are included in the Development Shell[fn:65] for Python[fn:64].
...
PlantUML Mode[fn:67] is a major mode for editing PlantUML[fn:66] sources in GNU/Emacs[fn:25].
...
Ivy (Swiper)[fn:68] is a generic completion mechanism for GNU/Emacs[fn:25]. While operating similarily to other completion schemes like =icomplete-mode=, it aims to be more efficient, smaller, simpler, and smoother to use, while remaining highly customizable.
...
It's possible to control the frame opacity in GNU/Emacs[fn:25]. Unlike other transparency hacks, it's not merely showing the desktop background image, but is true transparency -- you can se other windows behind the Emacs[fn:25] window.
...
The Desktop Environment[fn:69] package provides commands and a global minor mode for controlling your GNU/Linux[fn:8] desktop from within GNU/Emacs[fn:25].
...
Doom Themes[fn:70] is a theme megapack for GNU/Emacs[fn:25], inspired by community favourites.
...
[fn:1] https://laptopmedia.com/laptop-specs/acer-nitro-5-an515-53-2

[fn:2] https://raspberrypi.org/products/raspberry-pi-400/

[fn:3] https://www.raspberrypi.org/products/raspberry-pi-4-model-b/

[fn:4] https://nixos.org/manual/nixos/stable

[fn:5] https://samsung.com/us/mobile/galaxy-s10/buy/

[fn:6] https://github.com/t184256/nix-on-droid

[fn:7] https://x.org/wiki/

[fn:8] https://gnu.org

[fn:9] https://wayland.freedesktop.org

[fn:10] https://openssh.com

[fn:11] https://gohugo.io

[fn:12] https://nixos.wiki/wiki/Flakes

[fn:13] https://nixos.org/manual/nix/stable

[fn:14] https://nix-community.cachix.org

[fn:15] https://nixos.wiki/wiki/Nvidia

[fn:16] https://en.wikipedia.org/wiki/Firefox

[fn:17] https://jellyfin.org

[fn:18] https://moonlight-stream.org

[fn:19] https://teamviewer.com

[fn:20] https://github.com/nix-community/home-manager

[fn:21] https://git-scm.com

[fn:22] https://emacswiki.org/emacs/LearnEmacsLisp

[fn:23] https://neovim.io

[fn:24] https://gtk.org

[fn:25] https://gnu.org/software/emacs/

[fn:26] https://github.com/nixos/nix-mode

[fn:27] https://evil.readthedocs.io/en/latest/overview.html

[fn:28] https://github.com/ch11ng/exwm
...
[fn:32] https://gnu.org/software/emacs/erc.html

[fn:33] https://emacswiki.org/emacs/DiredMode

[fn:34] https://emacswiki.org/emacs/DiredExtra#Dired_X

[fn:35] https://github.com/domtronn/all-the-icons.el

[fn:36] https://github.com/iqbalansari/emacs-emojify

[fn:37] https://gnu.org/software/emacs/manual/html_mono/eshell.html

[fn:38] https://github.com/akermu/emacs-libvterm

[fn:39] https://github.com/neovim/libvterm

[fn:40] https://magit.vc

[fn:41] https://github.com/skeeto/elfeed

[fn:42] https://orgmode.org

[fn:43] https://github.com/org-roam/org-roam

[fn:44] https://zettelkasten.de

[fn:45] https://orgmode.org/worg/org-contrib/org-drill.html

[fn:46] https://github.com/marcinkoziej/org-pomodoro
...
[fn:50] https://wikipedia.org/wiki/Tex_Live

[fn:51] https://nixos.wiki/wiki/TexLive

[fn:52] https://github.com/zweifisch/ob-http

[fn:53] https://oxhugo.scripter.co

[fn:54] https://password-store.org

[fn:55] https://github.com/Silex/docker.el

[fn:56] https://github.com/spotify/dockerfile-mode

[fn:57] https://projectile.mx

[fn:58] https://microsoft.github.io/language-server-protocol

[fn:59] https://github.com/MaskRay/emacs-ccls

[fn:60] https://company-mode.github.io

[fn:61] https://emacswiki.org/emacs/GoMode

[fn:62] https://golang.org

[fn:63] https://plantuml.com

[fn:64] https://python.org

[fn:65] https://nixos.org/manual/nix/unstable/command-ref/nix-shell.html

[fn:66] https://github.com/skuro/plantuml-mode

[fn:67] https://github.com/abo-abo/swiper

[fn:68] https://github.com/DamienCassou/desktop-environment

[fn:69] https://github.com/hlissner/emacs-doom-themes

[fn:70] https://github.com/seagle0128/doom-modeline

[fn:71] https://heroku.com

[fn:72] https://grpc.io

[fn:73] https://iso.org/standard/74528.html

[fn:74] https://en.wikipedia.org/wiki/C++/

[fn:75] https://github.com/nix-community/emacs-overlay

[fn:76] https://github.com/nixos/nixos-hardware

[fn:77] https://nixos.org/manual/nixpkgs/stable

[fn:78] https://literateprogramming.com/knuthweb.pdf

[fn:79] https://openjdk.java.net

[fn:80] https://nodejs.org

[fn:81] https://rust-lang.org

[fn:82] https://docs.docker.com/machine/

[fn:83] https://docker.org

[fn:84] https://docs.docker.com/compose/

[fn:85] https://qemu.org
