...
The default host, built using QEMU[fn:1], a free and open-source emulator that can perform hardware virtualization. It features a lightweight system optimized for development, running GNU/Emacs[fn:2] + EXWM[fn:3] as the graphical environment.
...
The file system for this host is a single 24GB QCOW file, a format for disk images used by QEMU[fn:1]. The file can be recreated easily by following the steps listed in the NixOS[fn:4] installation manual, specifically the section on disk formatting.
...
My gaming laptop, the model is an Acer Nitro AN-515-53[fn:5]. The Nitro 5 has more in common with the mid-range notebooks rather than the gaming models due to its cooling design, chassis, and overall construction.
...
The Raspberry Pi 400[fn:6] is your complete personal computer, built into a compact keyboard. It features a quad-core, 64-bit processor, 4GB of RAM, wireless networking, dual-display output, 4k video playback, as well as a 40-pin GPIO header. It's the most powerful Raspberry Pi computer yet.
...
This is my Samsung Galaxy S10+[fn:8] running Nix On Droid[fn:9] with the experimental support for Flakes being used to manage the configuration.
...
X11, or X[fn:10] is the generic name for the X Window System Display Server. All graphical GNU/Linux[fn:11] applications connect to an X-Window[fn:10] (or Wayland[fn:12]) to display graphical data on the monitor of a computer. Its a program that acts as the interface between graphical applications and the graphics subsystem of the computer.
...
OpenSSH[fn:13] is a suite of secure networking utilities based on the Secure Shell Protocol, which provides a secure channel over an unsecured network in a client-server architecture. OpenSSH[fn:13] started as a fork of the free SSH program; later versions were proprietary software.
...
Hugo[fn:14] is one of the most popular open-source static site generators.
...
Nix Flakes[fn:15] are an upcoming feature of the Nix package manager[fn:16]. They allow you to specify your codes dependencies in a declarative way, simply by listing them inside of a ~flake.nix~ file. Each dependency is then pinned to a specific git-hash. Flakes[fn:15] replace the =nix-channels= command and things like ~builtins.fetchGit~, keeping dependencies at the top of the tree, and channels always in sync. Currently, Flakes[fn:15] are not available unless explicitly enabled.
...
Cachix[fn:17] is a Command line client for Nix[fn:16] binary cache hosting. This allows downloading and usage of pre-compiled binaries for applications on /nearly/ every available system architecture. This speeds up the time it takes to rebuild configurations.
...
Use the ~lspci~ command to determine the type of graphics card you have, following the guide on NVIDIA at the NixOS Wiki[fn:18].
...
Firefox Browser[fn:19], also known as Mozilla Firefox or simply Firefox, is a free and open-source web browser developed by the Mozilla Foundation and its subsidiary, the Mozilla Corporation. Firefox uses the Gecko layout engine to render web pages, which implements current and anticipated web standards. In 2017, Firefox began incorporating new technology under the code name Quantum to promote parallelism and a more intuitive user interface.
...
Moonlight[fn:21] is an open-source implementation of NVIDIA's GameStream Protocol. You can stream your collection of PC games from your GameStream-compatible PC to any supported device and play them remotely. Moonlight[fn:21] is perfect for gaming on the go (or on GNU/Linux[fn:11]) without sacrificing the graphics and game selection available for the PC.
...
The Teamviewer[fn:22] remote connectivity cloud platform enables secure remote access to any device, across platforms, from anywhere, anytime. Teamviewer[fn:22] connects computers, smartphones, servers, IoT devices, robots -- anything -- with fast, high performance connections through their global access network. It has been used in outer-space low-bandwidth environments.
...
Home Manager[fn:23] includes a =flake.nix= file for compatibility with Nix Flakes, a feature utilized heavily in this project. When using flakes, switching to a new configuration is done /only/ for the entire system, using the command ~nixos-rebuild switch --flake <path>~, instead of ~nixos-rebuild~, and ~home-manager~ seperately.
...
GNU Privacy Guard[fn:25] is a free-software replacement for Symantec's PGP cryptographic software suite. It is compliant with RFC 4880, the IETF standards-track specification of OpenPGP. Modern versions of PGP are interoperable with GnuPG and other OpenPGP-compliant systems.
...
Neovim[fn:26] is a project that seeks to aggressively refactor Vim in order to:
...
GTK[fn:27] is a free and open-source, cross-platform widget toolkit for graphical user interfaces. It's one of the most popular toolkits for the Wayland[fn:12] and X11[fn:10] windowing systems.
...
GNU/Emacs[fn:2] is an extensible, customizable, free/libre text editor -- and more. At its core is an interpreter for Emacs Lisp[fn:25], a dialect of the Lisp programming language with extensions to support text editing. Other features include:
...
Nix-mode[fn:28] is an Emacs[fn:2] major mode for editing Nix[fn:16] expressions. This provides basic handling of =.nix= files. Syntax highlighting and indentation support using =SMIE= are provided.
...
Evil[fn:29] is an extensible VI layer for GNU/Emacs[fn:2]. It emulates the main features of Vim[fn:26], turning GNU/Emacs[fn:2] into a modal editor.
...
The next time Emacs[fn:2] is started, it will come up in /normal state/, denoted by =<N>= in the modeline. This is where the main ~vi~ bindings are defined. Like Emacs[fn:2] in general, Evil[fn:29] is extensible in Emacs Lisp[fn:25].
...
EXWM (Emacs X Window Manager)[fn:3] is a full-featured tiling X window manager for GNU/Emacs[fn:2] built on-top of XELB. It features:
...
I wanted to leave ~(exwm-enable)~ out of my Emacs configuration (which does no harm anyways). This can be called when using the daemon to start EXWM[fn:3].
...
EXWM[fn:3] cannot make an X window manager by itself, this is by design; You must tell X to do it. Override the =~/.xinitrc= file to start the =xsession=.
...
General[fn:30] provides a more convenient method for binding keys in Emacs[fn:2], providing a unified interface for key definitions. Its primary purpose is to build on /existing/ functionality to make key definitions more clear and concise.
...
Which-key[fn:31] is a minor mode for Emacs[fn:2] that displays the key bindings following your currently entered incomplete command (prefix) in a popup or mini-buffer.
...
The Emacs Web Wowser[fn:32] is a Web browser written in Emacs Lisp[fn:25] based on the ~shr.el~ library. It's my primary browser when it comes to text-based browsing.
...
ERC[fn:33] is a powerful, modular, and extensible IRC client for GNU/Emacs[fn:2]. It's part of the GNU project, and included in Emacs.
...
EShell [fn:38] is a shell-like command interpreter for GNU/Emacs[fn:2] implemented in Emacs Lisp[fn:25]. It invokes no external processes except for those requested by the user. It's intended to be an alternative for IELM, and a full REPL envionment for Emacs[fn:2].
...
Emacs Libvterm (VTerm)[fn:39] is a fully-fledged terminal emulator inside GNU/Emacs[fn:2] based on Libvterm[fn:40], a blazing fast C library used in Neovim[fn:26]. As a result of using compiled code (instead of Emacs Lisp[fn:25]), VTerm[fn:39] is capable, fast, and it can seamlessly handle large outputs.
...
Magit[fn:41] is an interface to the Git[fn:24] version control system, implemented as a GNU/Emacs[fn:2] package written in Elisp[fn:25]. It fills the glaring gap between the Git[fn:24] command line interface and various GUIs, letting you perform trivial as well as elaborate version control tasks within a few mnemonic key presses.
...
Org Roam[fn:44] is a plain-text knowledge management system. It borrows principles from the Zettelkasten method[fn:45], providing a solution for non-hierarchical note-taking. It should also work as a plug-and-play solution for anyone already using Org Mode[fn:43] for their personal wiki (me).
...
Org Drill[fn:46] is an extension for Org Mode[fn:43] that uses a spaced repition algorithm to conduct interactive /Drill Sessions/ using Org files as sources of facts to be memorized.
...
Writegood Mode[fn:49] is an Emacs[fn:2] minor mode to aid in finding common writing problems. It highlights the text based on the following criteria:
...
GNU Aspell[fn:50] is a Free and Open Source spell checker designed to replace ISpell. It can be used as a library, or an independent spell checker. Its main feature is that it does a superior job of suggesting possible replacements for mis-spelled words than any other spell checker for the English language.
...
It's possible to make HTTP requests from Org Mode buffers using ob-http[fn:53], this relies on Org Babel being present and configured properly.
...
Ox-Hugo[fn:54] is an Org-Mode[fn:43] exporter for Hugo[fn:14] compabile markdown. My dotfiles are a result of this, and are available to view here https://chrishayward.xyz/dotfiles/.
...
Manage Docker from inside of Emacs using Docker.el[fn:56]. This is a full docker porcelain similar to Magit, allowing complete control of a Docker system. Add syntax highlighting to Dockerfiles using dockerfile-mode[fn:57] from Spotify.
...
Projectile[fn:58] is a project interaction library for GNU/Emacs[fn:2]. Its goal is to provide a nice set of features operating on a project level, without introducing external dependencies.
...
The Language Server Protocol (LSP)[fn:59] defines the protocol used between an Editor or IDE, and a language server that provides features like:
...
Emacs CCLS[fn:60] is a client for CCLS, a C/C++/Objective-C language server supporting multi-million line C++ code bases, powered by libclang.
...
Go Mode[fn:62] is a major mode for editing Golang[fn:63] source code in GNU/Emacs[fn:2].
...
Rustic[fn:61] is a fork of rust-mode that integrates well with the Language Server Protocol[fn:55]. Include the rust shell before launching GNU/Emacs[fn:2] to use this!
...
The built in Python Mode[fn:64] has a nice feature set for working with Python[fn:65] code in GNU/Emacs[fn:2]. It is complimented with the addition of an LSP[fn:59] server. These tools are included in the Development Shell[fn:66] for Python[fn:65].
...
PlantUML[fn:67] is an open-source tool allowing users to create diagrams from a plain-text language. Besides various UML diagrams, PlantUML[fn:67] has support for various other software developmented related formats, as well as visualizations of =JSON= and =YAML= files.
...
PlantUML Mode[fn:68] is a major mode for editing PlantUML[fn:67] sources in GNU/Emacs[fn:2].
...
Ivy (Swiper)[fn:69] is a generic completion mechanism for GNU/Emacs[fn:2]. While operating similarily to other completion schemes like =icomplete-mode=, it aims to be more efficient, smaller, simpler, and smoother to use, while remaining highly customizable.
...
The Desktop Environment[fn:70] package provides commands and a global minor mode for controlling your GNU/Linux[fn:11] desktop from within GNU/Emacs[fn:2].
...
Doom Modeline[fn:71] is a fancy and fast modeline inspired by minimalism design. It's integrated into Centaur Emacs, Doom Emacs, and Spacemacs.
...
[fn:3] https://github.com/ch11ng/exwm

[fn:4] https://nixos.org/manual/nixos/stable

[fn:5] https://laptopmedia.com/laptop-specs/acer-nitro-5-an515-53-2

[fn:6] https://raspberrypi.org/products/raspberry-pi-400/

[fn:7] https://www.raspberrypi.org/products/raspberry-pi-4-model-b/

[fn:8] https://samsung.com/us/mobile/galaxy-s10/buy/

[fn:9] https://github.com/t184256/nix-on-droid

[fn:10] https://x.org/wiki/

[fn:11] https://gnu.org

[fn:12] https://wayland.freedesktop.org

[fn:13] https://openssh.com

[fn:14] https://gohugo.io

[fn:15] https://nixos.wiki/wiki/Flakes

[fn:16] https://nixos.org/manual/nix/stable

[fn:17] https://nix-community.cachix.org

[fn:18] https://nixos.wiki/wiki/Nvidia

[fn:19] https://en.wikipedia.org/wiki/Firefox

[fn:20] https://jellyfin.org

[fn:21] https://moonlight-stream.org

[fn:22] https://teamviewer.com

[fn:23] https://github.com/nix-community/home-manager

[fn:24] https://git-scm.com

[fn:25] https://emacswiki.org/emacs/LearnEmacsLisp

[fn:26] https://neovim.io

[fn:27] https://gtk.org

[fn:28] https://github.com/nixos/nix-mode

[fn:29] https://evil.readthedocs.io/en/latest/overview.html

[fn:30] https://github.com/noctuid/general.el

[fn:31] https://github.com/justbur/emacs-which-key

[fn:32] https://emacswiki.org/emacs/eww

[fn:33] https://gnu.org/software/emacs/erc.html
...
[fn:46] https://orgmode.org/worg/org-contrib/org-drill.html

[fn:47] https://github.com/marcinkoziej/org-pomodoro

[fn:48] https://en.wikipedia.org/wiki/Pomodoro_Technique

[fn:49] https://github.com/bnbeckwith/writegood-mode

[fn:50] https://aspell.net

[fn:51] https://wikipedia.org/wiki/Tex_Live

[fn:52] https://nixos.wiki/wiki/TexLive

[fn:53] https://github.com/zweifisch/ob-http

[fn:54] https://oxhugo.scripter.co

[fn:55] https://password-store.org

[fn:56] https://github.com/Silex/docker.el

[fn:57] https://github.com/spotify/dockerfile-mode

[fn:58] https://projectile.mx

[fn:59] https://microsoft.github.io/language-server-protocol

[fn:60] https://github.com/MaskRay/emacs-ccls

[fn:61] https://company-mode.github.io

[fn:62] https://emacswiki.org/emacs/GoMode

[fn:63] https://golang.org

[fn:64] https://plantuml.com

[fn:65] https://python.org

[fn:66] https://nixos.org/manual/nix/unstable/command-ref/nix-shell.html

[fn:67] https://github.com/skuro/plantuml-mode

[fn:68] https://github.com/abo-abo/swiper

[fn:69] https://github.com/DamienCassou/desktop-environment

[fn:70] https://github.com/hlissner/emacs-doom-themes

[fn:71] https://github.com/seagle0128/doom-modeline

[fn:72] https://docs.docker.com/compose/

[fn:73] https://docker.org

[fn:74] https://docs.docker.com/machine/

[fn:75] https://rust-lang.org

[fn:76] https://nodejs.org

[fn:77] https://openjdk.java.net

[fn:78] https://literateprogramming.com/knuthweb.pdf

[fn:79] https://nixos.org/manual/nixpkgs/stable

[fn:80] https://github.com/nixos/nixos-hardware

[fn:81] https://github.com/nix-community/emacs-overlay

[fn:82] https://en.wikipedia.org/wiki/C++/

[fn:83] https://iso.org/standard/74528.html

[fn:84] https://grpc.io

[fn:85] https://heroku.com
